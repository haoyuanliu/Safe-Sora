...
def get_parser():
    ...
    parser.add_argument("--lambda_w", type=float, default=0.05)
    ...
    parser.add_argument("--output_dir", default="./output-only-bit")
...
for epoch in tqdm(range(1, args.epochs + 1), desc="Epochs", unit="epoch", ncols=100):
    ...
    for batch_idx, (video, logo_image) in enumerate(stream):
        ...
        extracted_wm_bit, str_extracted_wm_bit = qipangeutil.extract(watermark, 0.6)
        ...
        if batch_idx%200==0:
            save_image(watermark, os.path.join("results-only-bits", f"watermark_epoch{epoch}_batch{batch_idx}.png"))
            save_mul_video([reconst_video, reconst_video_w, abs(reconst_video - reconst_video_w) * 5], os.path.join('results-only-bits',   
