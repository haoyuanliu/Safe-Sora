...
for epoch in tqdm(range(1, args.epochs + 1), desc="Epochs", unit="epoch", ncols=100):
    ...
    for batch_idx, (video, logo_image) in enumerate(stream):
        ...
        extracted_wm_bit, str_extracted_wm_bit = qipangeutil.extract(watermark, 0.1)
        ...
        loss = loss_video + args.lambda_w * loss_wm + args.lambda_w * loss_wm_bits
        ...
        if batch_idx%200==0:
            save_image(watermark, os.path.join("results", f"watermark_epoch{epoch}_batch{batch_idx}.png"))
            save_mul_video([reconst_video, reconst_video_w, abs(reconst_video - reconst_video_w) * 5], os.path.join('results',   
